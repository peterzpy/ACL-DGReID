...
class Sequential_ext(nn.Module):
    ...
    def forward(self, input):
        ...
        for i, module in enumerate(self._modules.values()):
            input, output_origin, output_sub = module(input)
            out_origin_features.append(output_origin)
            out_sub_features.append(output_sub)
        return input, out_origin_features, out_sub_features
...
class MixNorm(nn.Module):
    def __init__(self, num_features, bn_norm):
        super(MixNorm, self).__init__()
        self.bn = get_norm(bn_norm, num_features)
        self.ibn = IBN(num_features, bn_norm, affine=True)
        self.proj = nn.Conv2d(num_features*2, num_features, 1, bias=True)
        self.relu = nn.ReLU()
        
    def forward(self, x):
        x = torch.cat([self.bn(x), self.ibn(x)], 1)
...
class StyleNorm(nn.Module):
    def __init__(self, num_features):
        super(StyleNorm, self).__init__()
        self.IN = nn.InstanceNorm2d(num_features)
        self.fc = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Conv2d(num_features, num_features//16, 1, bias=True),
            nn.ReLU(),
            nn.Conv2d(num_features//16, num_features, 1, bias=True),
            nn.Sigmoid()
        )
        self.relu = nn.ReLU()
        
    def forward(self, x):
        IN_feature = self.IN(x)
        residual_feature = x - IN_feature
        weight = self.fc(residual_feature)
        return IN_feature + residual_feature * weight, IN_feature, IN_feature + residual_feature * (1 - weight)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, bn_norm, with_ibn=False, with_se=False,
        ...
        if with_ibn:
            self.bn1 = IBN(planes, bn_norm, affine=True)
        else:
            self.bn1 = get_norm(bn_norm, planes)
