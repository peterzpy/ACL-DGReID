...
def _pluck(dataset_name, pre_path, identities, indices, relabel=False):
    ret = []
    for index, pid in enumerate(indices):
        ...
        for camid, cam_images in enumerate(pid_images):
            for fname in cam_images:
                ...
                if relabel:
                    ret.append((osp.join(pre_path, fname), dataset_name + "_" + str(index), dataset_name + "_" + str(camid)))
                else:
                    ret.append((osp.join(pre_path, fname), dataset_name + "_" + str(pid), dataset_name + "_" + str(camid)))
    return ret

def _pluck_gallery(dataset_name, pre_path, identities, indices, relabel=False):
    ret = []
    for index, pid in enumerate(indices):
        pid_images = identities[pid]
        for camid, cam_images in enumerate(pid_images):
            if len(cam_images[:-1])==0:
                for fname in cam_images:
                    ...
                    assert pid == x and camid == y
                    if relabel:
                        ret.append((fname, dataset_name + "_" + str(index), dataset_name + "_" + str(camid)))
                    else:
                        ret.append((fname, dataset_name + "_" + str(pid), dataset_name + "_" + str(camid)))
            else:
...
def _pluck_query(dataset_name, pre_path, identities, indices, relabel=False):
    ret = []
    for index, pid in enumerate(indices):
        pid_images = identities[pid]
        for camid, cam_images in enumerate(pid_images):
...
class CUHK03FULL(ImageDataset):
    ...
    def __init__(self, root='datasets', split_id=0, cuhk03_labeled=True, cuhk03_classic_split=False, **kwargs):
        root='/data/pengyi/datasets/reid_data/cuhk03/cuhk03_release'
        self.root = root
        ...
        train, query, gallery = self.load()

        super(CUHK03FULL, self).__init__(train, query, gallery, **kwargs)
    ...
    def load(self):
        ...
        self.pre_path = osp.join(self.root, 'images')
        train = _pluck(self.dataset_name, self.pre_path, identities, train_pids, relabel=True)
        query = _pluck_query(self.dataset_name, self.pre_path, identities, self.split['query'])
        #self.gallery = _pluck(identities, self.split['gallery'])
        gallery = _pluck_gallery(self.dataset_name, self.pre_path, identities, self.split['gallery'])
